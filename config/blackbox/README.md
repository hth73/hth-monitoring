...
- job_name: 'blackbox-http-check'
  ...
  static_configs:
    - targets:
      - https://caddy.htdom.local/metrics
      - https://grafana.htdom.local/metrics
      - https://loki.htdom.local/ready
      - https://prometheus.htdom.local/metrics
  relabel_configs:
    - source_labels: [__address__]
      target_label: __param_target
    - source_labels: [__param_target]
...
avg(scrape_duration_seconds{job="blackbox-http-check", instance=~"https://mina.htdom.local/metrics"})
...
(probe_ssl_earliest_cert_expiry{job=~"blackbox-http-check", instance=~"https://mina.htdom.local/metrics"} - time()) / 3600 / 24

avg_over_time(probe_duration_seconds{job=~"blackbox-http-check", instance=~"https://mina.htdom.local/metrics"}[1m])
